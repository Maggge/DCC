...
void DCC::begin(const FSH * motorShieldName, MotorDriver * mainDriver, MotorDriver* progDriver) {
  ...
  StringFormatter::send(Serial,F("<iDCC-EX / %S / %S>\n"), F(ARDUINO_TYPE), shieldName);
...
void DCC::setThrottle(uint16_t cab, uint8_t tSpeed, uint8_t tSpeedSteps, bool tDirection){
  setThrottle(cab, tSpeed, tDirection);
  int reg = lookupSpeedTable(cab);
  if(cab <= 0) return;
  speedTable[reg].speedSteps = tSpeedSteps;
}
...
unsigned long DCC::getFns(int cab){
  if(cab <= 0) return 0;
  int reg = lookupSpeedTable(cab);
...
uint8_t DCC::getSpeedSteps(int cab){
  if(cab <= 0) return 0;
  int reg = lookupSpeedTable(cab);
  if(reg <= 0) return 0;
  return speedTable[reg].speedSteps;
}